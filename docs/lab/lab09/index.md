---
layout: md
title: 上机作业9
---

<h1>上机作业9</h1>

## 1. 抽象的练习
[Person.zip](../../../_includes/zip/lab/lab09/Person.zip)
主类名:lab.lab09.Person.Main

{% highlight java linenos %}
{% include lab/lab09/Person/Person.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/Person/Teacher.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/Person/Worker.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/Person/Main.java %}
{% endhighlight %}

## 2. 接口的作业题-第16章
[PersonInterface.zip](../../../_includes/zip/lab/lab09/PersonInterface.zip)
主类名:lab.lab09.Student.Main

{% highlight java linenos %}
{% include lab/lab09/PersonInterface/Person.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/PersonInterface/Teacher.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/PersonInterface/Worker.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/PersonInterface/Movable.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/PersonInterface/Soundable.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/PersonInterface/Main.java %}
{% endhighlight %}

## 3. 接口编程一
[Student.zip](../../../_includes/zip/lab/lab09/Student.zip)
主类名:lab.lab09.Student.Test

{% highlight java linenos %}
{% include lab/lab09/Student/Student.java %}
{% endhighlight %}

{% highlight javalinenos %}
{% include lab/lab09/Student/Test.java %}
{% endhighlight %}

## 4. 异常处理编程-2
[Time.zip](../../../_includes/zip/lab/lab09/Time.zip)
主类名:lab.lab09.Time.Test

{% highlight java linenos %}
{% include lab/lab09/Time/TimeFormatException.java %}
{% endhighlight %}

{% highlight java linenos %}
{% include lab/lab09/Time/Test.java %}
{% endhighlight %}